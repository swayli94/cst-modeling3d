...
if __name__ == "__main__":
    ...
    # Trailing edge upper
    xTE1 = np.array([20.926, 25.423, 28.316, 30.028, 32.158, 33.512, 33.857])
    yTE1 = np.array([-3.837, -0.504,  2.591,  5.010,  9.223, 13.689, 17.154])
    zTE1 = np.array([-3.425, -2.192, -1.199, -0.539,  0.446,  1.353,  9.067])

    # Trailing edge lower
    xTE2 = np.array([20.996, 25.411, 28.208, 29.859, 31.902, 33.181, 33.931])
    yTE2 = np.array([-2.382,  0.356,  3.467,  5.865, 10.032, 14.473, 17.118])
    zTE2 = np.array([-3.425, -2.215, -1.219, -0.565,  0.427,  1.328,  6.731])
    ...
    with open('ori-sections.dat', 'r') as f:
        ...
        for i in range(n_sec):
            ...
            print('Section %d ---------------'%(i+1))
            for ii in range(coef_upp.shape[0]):
                print(' %10.6f '%(coef_upp[ii]), end='')
            print()
            for ii in range(coef_low.shape[0]):
